...
class BaseModel(nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        # if x.size(1)==6:
        if 'channels' in self.yaml and self.yaml['channels'] == 6:
            rgb,ir=torch.chunk(x,chunks=2,dim=1) # 分离可见光和红外
            x=rgb
            ...
            for m in self.model:
                # try:
                #     print(f'第{m.i}层：{rgb.shape}')
                # except Exception:
                #     print(f'第{m.i}层')
                # 既不是ADD 也不是 Fusion 也不为-1
                if m.f != -1 and m.f != -4 and m.f != -3:  
                ...
                #处理CAFusion模块
                    ...
                    if texture is None and isinstance(m, TextureInference):
                        texture = m(rgb,ir)
                    elif(len(list(m.named_parameters()))>0) and texture is not None:
                        rgb,ir = m(rgb,ir,texture)
                        x=[]# 清空中间层
                    else :
                        rgb,ir = m(rgb,ir)
                        x=[]# 清空中间层
                ...
                # elif m.i<20:
                elif m.i < len(self.yaml['backbone']):
                    if isR:
                        x= m(rgb)
                        rgb=x
                    else :
            ...
            max_idx = max(embed)
            for m in self.model:
                # print(f'原始代码{x.shape}')
                # print(f'第{m.i}层：{x.shape}')
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            c1, c2 = ch[f], args[0]# 输入通道来自前一层输出，输出通道来自args[0]
            if f==-4:
                c1=tx[ty]
                if ty!=0:
                    c1=c1*width
            ...
            args = [c1, args[0]]
        elif m is TextureInference:
            c2 = 3  #实际该模块输出通道为1，但为了适配后续模块，暂时设置为3
...
#            print("ch[f]", f, ch[f[0]])
            ...
            # 处理 CAFusion 模块
            args = [int(args[0]*width), int(args[1])]
