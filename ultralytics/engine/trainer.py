...
class BaseTrainer:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        if "train_ir" in self.data:
            self.trainset, self.testset,self.trainirset,self.testirset = \
                self.data["train"], self.data.get("val"), self.data["train_ir"], self.data.get("val_ir") or self.data.get("test")
        self.ema = None
    ...
    def _setup_train(self, world_size):
        ...
        # 读取数据集
        if hasattr(self, 'trainirset'):
            self.train_loader = self.get_dataloader2(self.trainset, self.trainirset,batch_size=batch_size, rank=RANK, mode="train")
        else:
            self.train_loader = self.get_dataloader(self.data["train"], batch_size=batch_size, rank=LOCAL_RANK, mode="train")
        if RANK in {-1, 0}:
            # Note: When training DOTA dataset, double batch size could get OOM on images with >2000 objects.
            if hasattr(self, 'testirset'):
                self.test_loader = self.get_dataloader2(
                    self.testset, self.testirset,
                    batch_size=batch_size if self.args.task == "obb" else batch_size * 2, rank=-1, mode="val"
                )
            else:
