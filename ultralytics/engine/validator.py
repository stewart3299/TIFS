...
class BaseValidator:
    ...
    def __call__(self, trainer=None, model=None):
        ...
        if self.training:
            ...
            self.args.half = self.device.type != "cpu" and trainer.amp  # force FP16 val during training
            ...
            if strir in self.data:
                self.dataloader = self.dataloader or self.get_dataloader2(self.data.get(self.args.split), self.data.get(strir), self.args.batch)
                model.eval()
                for i in range(1000):
    ...
    def get_dataloader2(self, dataset_path, datasetir_path, batch_size):
        """Get data loader from dataset path and batch size."""
        raise NotImplementedError("get_dataloader2 function not implemented for this validator")
